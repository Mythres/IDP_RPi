...
class Transportrebuild:
    def __init__(self):
        self.name = "transportrebuild"
        self.conn = None
...
transportrebuild = None


def name():
    return transportrebuild.name


def load():
    global transportrebuild
    transportrebuild = Transportrebuild()
...
def start(conn):
    try:
        transportrebuild.run(conn)
    except KeyboardInterrupt:
        transportrebuild.unload()
        sys.exit(1)
